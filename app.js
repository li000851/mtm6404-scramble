...
function ScrambleGame() {
  // Message colors
  const messageColors = {
    "You passed. Next word.": "lightblue",
    "Correct! Next word.": "lightgreen",
    "Wrong. Try again!": "lightcoral",
    "Game Over!": "tomato",
  ...
  return (
    <div>
      ...
      <div id="container">
        <button onClick={resetGame} className="start">Start</button>
        ...
        {/* apply bg color */}
        <p style={{
          backgroundColor: messageColors[message] || 'transparent',
          padding: '5px',
          borderRadius: '5px'
        }}>
          {message}
        ...
        <button className="pass" onClick={passDealer} disabled={pass <= 0}>Pass</button>
        <div>
          <p>Points: {score}</p>
          <p>Strikes: {strike}</p>
        </div>
